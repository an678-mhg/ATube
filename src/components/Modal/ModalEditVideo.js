...
const ModalEditVideo = ({ setShow, video }) => {
  ...
  const handleEditVideo = async (e) => {
    ...
    if (data.title.trim().length > 76 || data.description.trim().length > 100) {
      return toast.error(
  ...
  return (
    <Overlay setShow={setShow}>
      <form
        ...
        className="bg-[#222] w-[400px] max-w-[calc(100%-32px)] p-4"
      >
        <div className="w-full aspect-[16/9] mb-4 relative">
          ...
          <img className="w-full h-full object-cover" src={previewThumnail} />
          <input type={"file"} hidden id="thumnail" onChange={onChangeFile} />
        </div>
        <div className="mb-4">
